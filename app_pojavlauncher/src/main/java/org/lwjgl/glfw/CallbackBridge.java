            ...
            case NOTIF_TYPE_SDL:
                if (action[0] == ACTION_INIT_LAUNCHER_INTEGRATION) {
                    try {
                        ...
                        Logger.appendToLog("PengusCraft: SDL support enabled!");
                        return true;
                    } catch (Exception e){
                        Logger.appendToLog("PengusCraft: Failed to initialize SDL launcher-side integration! We will likely crash");
                    }
                }
