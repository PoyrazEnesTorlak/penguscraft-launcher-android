...
public final class Tools {
    ...
    public static String APP_NAME = "PenguLauncher";
    ...
    public static String DIR_GAME_HOME = Environment.getExternalStorageDirectory().getAbsolutePath() + "/games/PenguLauncher";
    ...
    private static File getPojavStorageRoot(Context ctx) {
        if(SDK_INT >= 29) {
            ...
            return new File(Environment.getExternalStorageDirectory(),"games/PenguLauncher");
        }
    }
    ...
    public static void launchMinecraft(final AppCompatActivity activity, MinecraftAccount minecraftAccount,
        ...
        // Pengu: sunucu kilidi modu adresi buradan okuyor
        if (net.kdt.pojavlaunch.pengu.PenguHazirlik.pengusProfiliMi(minecraftProfile)) {
            javaArgList.add("-Dpenguscraft.server=" + net.kdt.pojavlaunch.pengu.PenguConfig.SUNUCU_ADRES);
            javaArgList.add("-Dminecraft.launcher.brand=PengusCraft");
            javaArgList.add("-Dminecraft.launcher.version=" + BuildConfig.VERSION_NAME);
        }
        javaArgList.add(versionInfo.mainClass);
        javaArgList.addAll(Arrays.asList(launchArgs));
        // Pengu: oyun acilir acilmaz dogrudan sunucuya baglan (1.20+ quick play)
        if (net.kdt.pojavlaunch.pengu.PenguHazirlik.pengusProfiliMi(minecraftProfile)) {
            javaArgList.add("--quickPlayMultiplayer");
            javaArgList.add(net.kdt.pojavlaunch.pengu.PenguConfig.SUNUCU_ADRES);
        }
        ...
        if (hasLegacy4J) {
            ...
            oldL4JMitigationLogListener = loggedLine -> {
                if (loggedLine.contains("literal{SDL3 (isXander's libsdl4j)} isn't supported in this system. GLFW will be used instead.")) {
                    Logger.appendToLog("PengusCraft: Broken version of Legacy4J (below 1.8.51.8.5.2537.1) detected! Force enabling SDL");
                    ...
                    Logger.appendToLog("PengusCraft: Fixed version of Legacy4J (1.8.5.2537.1 or higher) detected! Have fun!");
                    Logger.removeLogListener(oldL4JMitigationLogListener);
                }
            };
